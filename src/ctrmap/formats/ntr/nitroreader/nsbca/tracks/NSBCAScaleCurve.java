...
public class NSBCAScaleCurve extends NSBCAScaleTrack {
    private int flags;
    private int offset;

    public NSBCAScaleCurve(NTRDataIOStream data) throws IOException {
        this.flags = data.readInt();
        this.offset = data.readInt();
    }

	@Override
    public float[] getData(NTRDataIOStream data, int JAC_offset, int frameCount) throws IOException {
        boolean isFX16 = NNSAnimationTrackFlags.isFX16Skl(flags);

		int valueCount = Utils.getValueCount(frameCount, getFrameStep());
